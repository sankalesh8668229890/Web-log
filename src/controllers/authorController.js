...
let createauthor = async function (req, res) {
    try {
        ...
        // REGEX VALIDATION
        if (!data.firstName.match(regex)) return res.status(400).send({ status: false, msg: "FIRSTNAME SHOULD ONLY CONATIN ALPHABATS AND LENTH MUST BE IN BETWEEN 2-30" })
        if (!data.lastName.match(regex)) return res.status(400).send({ status: false, msg: "LASTNAME SHOULD ONLY CONATIN ALPHABATS AND LENTH MUST BE IN BETWEEN 2-30" })
